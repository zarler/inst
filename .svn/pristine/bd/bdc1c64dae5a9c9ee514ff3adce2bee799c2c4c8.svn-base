...
class Controller_Ver1_CreditInfo extends AppCore {
    ...
    public function action_WorkInfo(){
        ...
        $work = $this->model['work']->get_one(App::$_token['user_id']);
        if(( $work && $work['status'] != Model_CreditInfo_Work::STATUS_INVALID) ||
        ...
        Model::factory('CreditInfo_Step')->change(App::$_token['user_id'],['status' => Model_CreditInfo_Step::COMPLETE,'pass_time' => time()],Model_CreditInfo_Step::WORK);
    ...
    public function action_HomeInfo(){
        ...
        if(!(Valid::not_empty($home_province) && Valid::not_empty($home_city) && Valid::not_empty($home_county) &&
            Valid::not_empty($home_address) )){
            $this->response_json(array(),"4005","参数错误");
        }
        $this->model['home'] = Model::factory("CreditInfo_Home");
        $home = $this->model['home']->get_one(App::$_token['user_id']);
        if(($home && $home['status'] ==Model_CreditInfo_Home::STATUS_INVALID)||
        ...
        Model::factory('CreditInfo_Step')->change(App::$_token['user_id'],['status' => Model_CreditInfo_Step::COMPLETE,'pass_time' => time()],Model_CreditInfo_Step::HOME);
        $this->response_json(array(),"1000","提交成功");
    }
    ...
    public function action_Contact(){
        ...
        if($create){
            Model::factory('CreditInfo_Step')->change(App::$_token['user_id'],['status' => Model_CreditInfo_Step::COMPLETE,'pass_time' => time()],Model_CreditInfo_Step::CONTACT);
            $this->response_json(array(),"1000","提交成功");
        }
    ...
    public function action_FaceIDAuth(){
        ...
        $issued = isset($this->_data['issued']) ? $this->_data['issued'] : '';
        $valid_date = isset($this->_data['valid_date']) ? $this->_data['valid_date'] : '';
        ...
        $res5 = HttpClient::factory($config['ps_url'] . $this->server)
            ...
            ->httpheader(array("CLIENTID:".$config['client_id'], "CLIENTSIGN:".md5(md5(file_get_contents($_FILES['ocr_pic']['tmp_name'])).$config['client_key'])))
            ->execute()
            ->body();
        ...
        $res6 = HttpClient::factory($config['ps_url'] . $this->server)
            ...
            ->httpheader(array("CLIENTID:".$config['client_id'], "CLIENTSIGN:".md5(md5(file_get_contents($_FILES['ocr_pic_back']['tmp_name'])).$config['client_key'])))
            ->execute()
            ->body();
            ...
            if($res && $rjson = json_decode($res,TRUE)){
                if( isset($rjson['status']) && $rjson['status']===TRUE ){
                    if(isset($rjson['api_result']['result']) && (string)$rjson['api_result']['result'] == Lib_Common::LIB_COMMON_API_RESULT_SUCCESS){//验证成功
                        $identity_face = isset($rjson['api_result']['info']['identity_face']) && $rjson['api_result']['info']['identity_face'] ? $rjson['api_result']['info']['identity_face'] : '' ;
        ...
        //网纹对比返回分数
        $score = $rbs['api_result']['info']['confidence'];
        //计算对比分数 获得审核状态
        ...
        try{
            $this->model['faceidauth']->update(
                ...
                array(
                    ...
                    'identity_req_data' => json_encode($rbs['api_result']['info']),
                ));
...
//        }
        ...
        if( $redis->set('inst_phone_add_'.App::$_token['user_id'],array('user_id'=>App::$_token['user_id'],'phonebook'=>json_encode($phonebook2))) ){
            Model::factory('CreditInfo_Step')->change(App::$_token['user_id'],['status' => Model_CreditInfo_Step::COMPLETE,'pass_time' => time()],Model_CreditInfo_Step::PHONEBOOK);
...
//        }
        ...
        $redis = Redis_Hash::instance();
        if( $redis->set('inst_sms_add_'.App::$_token['user_id'],array('user_id'=>App::$_token['user_id'],'message'=>json_encode($message))) ){
            Model::factory('CreditInfo_Step')->change(App::$_token['user_id'],['status' => Model_CreditInfo_Step::COMPLETE,'pass_time' => time()],Model_CreditInfo_Step::SMS);
...
//        }
        ...
        $redis = Redis_Hash::instance();
        if( $redis->set('inst_call_add_'.App::$_token['user_id'],array('user_id'=>App::$_token['user_id'],'history_list'=>json_encode($history_list))) ){
            Model::factory('CreditInfo_Step')->change(App::$_token['user_id'],['status' => Model_CreditInfo_Step::COMPLETE,'pass_time' => time()],Model_CreditInfo_Step::CALL);
            $this->response_json(array(),"1000","提交成功");
        }else{
        ...
        if(is_array($res)){
            if(isset($res['api_result']['html']) && $res['api_result']['html'] ){
                $this->response_json([
                    ...
                    'jump' => (string)$res['api_result']['jump'],
                    'sdk' => '0'
                ],'1000','申请已生成');
                ...
                $this->response_json(['jump' => '0'],'1000','申请关闭');
            }
        }else{
    ...
    public function action_List(){
        ...
        if(in_array(App::$_user['credit_auth'], [Model_User::CREDIT_AUTH_BASE_BACK,Model_User::CREDIT_AUTH_BASE_SUBMITED,Model_User::CREDIT_AUTH_BASE_CHECKING])) {
            $credit_msg = array(
                'status' => '0',
                'info' => "您已提交记录，请等待审核"
            );
        ...
        if(App::$_user['credit_auth']== Model_User::CREDIT_AUTH_BASE_VERIFIED ){
            ...
            $pass = Model::factory("CreditInfo_Step")->get_pass(App::$_token['user_id']);
        ...
        //拒绝增加30天有效期，拒绝后30天内不能再提交授信申请
        if(App::$_user['credit_reject_expire'] > 0 && time() < App::$_user['credit_reject_expire'])
            $foot_button['enable'] = '0';
