...
class Tool_CSV {
    ...
    public static function export_array($filename=NULL,$array=NULL){
            ...
            if(isset($array['body']) && count($array['body'])>0) {
                foreach($array['body'] as $row) {
                    if (isset($array['field'])) {
                        ...
                        foreach($array['field'] as $col){//按 field 顺序将每行填充
                            if(isset($row[$col])) {
                                $content .=  $content ? ',' . self::filter($row[$col]) : self::filter($row[$col]);
                            }
                        }

                    } else {
                        foreach($row as $value){
                            $content .= $content ? ',' . self::filter($value) : self::filter($value);
                        }
                    }
                }
            }
        ...
        self::export_csv($filename,iconv('utf-8','gb2312',$head.$content));
