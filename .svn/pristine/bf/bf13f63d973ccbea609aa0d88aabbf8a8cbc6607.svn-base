...
//        $this->model['newapp2coupon'] = Model::factory('Activity_NewApp2Coupon');
    ...
    public function action_Step()
    {
        ...
        if ($ca) {
            ...
            if (!isset($response_array['has_fastloan_order']) || $response_array['has_fastloan_order'] != "1") {
                ...
                if ($this->model['order']->firstByUserId(App::$_token['user_id'])) {
    ...
    public function action_MoxieTaobao()
    {
        ...
        if (!isset($this->step[Model_CreditInfo_Step::TAOBAO])){
        $this->model['step']->create(App::$_token['user_id'],Model_CreditInfo_Step::TAOBAO);
    }

        if (isset($this->step[Model_CreditInfo_Step::TAOBAO]) && (int)$this->step[Model_CreditInfo_Step::TAOBAO] == (int)Model_CreditInfo_Step::COMPLETE) {
    ...
    public function action_MoxieJD()
    {
        ...
        if (!isset($this->step[Model_CreditInfo_Step::JINGDONG])){
            $this->model['step']->create(App::$_token['user_id'],Model_CreditInfo_Step::JINGDONG);
        }

        if (isset($this->step[Model_CreditInfo_Step::JINGDONG]) && (int)$this->step[Model_CreditInfo_Step::JINGDONG] == (int)Model_CreditInfo_Step::COMPLETE) {
            $this->response_json([], "4002", "记录已存在");
        }
        $userId = date('YmdHis').Text::random(null, 4);
    ...
    public function action_MoxieBank()
    {
        ...
        if (!isset($this->step[Model_CreditInfo_Step::BANK])){
            $this->model['step']->create(App::$_token['user_id'],Model_CreditInfo_Step::BANK);
        }

        if (isset($this->step[Model_CreditInfo_Step::BANK]) && (int)$this->step[Model_CreditInfo_Step::BANK] == (int)Model_CreditInfo_Step::COMPLETE) {
            $this->response_json([], "4002", "记录已存在");
        }
        $userId = date('YmdHis').Text::random(null, 4);
        ...
        $a = $this->model['moxie_data_item']->add_data_item([
            'extends' => App::$_token['user_id'], 'userid' => $userId,
            'state' => 1,
        ]);


        $rs = $this->model['Risk_QueryRecord']->get_by_array([
            'user_id' => App::$_token['user_id'], 'provider' => 'Moxie','action'=>Model_CreditInfo_Step::BANK
        ], 11);
        if (count($rs) >= 10) {
            $this->response_json([], "4006", "重试次数已超上限");
        }
        ...
        $this->model['Risk_QueryRecord']->create([
            'user_id' => App::$_token['user_id'], 'provider' => 'Moxie','action'=>Model_CreditInfo_Step::BANK, 'status' => 1,
        ]);
    ...
    public function action_MoxieChsi()
    {
        ...
        if (!isset($this->step[Model_CreditInfo_Step::CHSI])){
            $this->model['step']->create(App::$_token['user_id'],Model_CreditInfo_Step::CHSI);
        }

        if (!isset($this->step[Model_CreditInfo_Step::CHSI])){
            $this->model['step']->create(App::$_token['user_id']);
        }
        if (isset($this->step[Model_CreditInfo_Step::CHSI]) && (int)$this->step[Model_CreditInfo_Step::CHSI] == (int)Model_CreditInfo_Step::COMPLETE) {
            $this->response_json([], "4002", "记录已存在");
        }
    ...
    public function action_MoxieFund()
    {
        ...
        if (!isset($this->step[Model_CreditInfo_Step::FUND])){
            $this->model['step']->create(App::$_token['user_id'],Model_CreditInfo_Step::FUND);
        }

        if (isset($this->step[Model_CreditInfo_Step::FUND]) && (int)$this->step[Model_CreditInfo_Step::FUND] == (int)Model_CreditInfo_Step::COMPLETE) {
            $this->response_json([], "4002", "记录已存在");
        }
        $userId = date('YmdHis').Text::random(null, 4);
    ...
    public function action_MoxieSocialSecurity()
    {
        ...
        if (!isset($this->step[Model_CreditInfo_Step::SOCIAL_SECURITY])){
            $this->model['step']->create(App::$_token['user_id'],Model_CreditInfo_Step::SOCIAL_SECURITY);
        }

        if (isset($this->step[Model_CreditInfo_Step::SOCIAL_SECURITY]) && (int)$this->step[Model_CreditInfo_Step::SOCIAL_SECURITY] == (int)Model_CreditInfo_Step::COMPLETE) {
            $this->response_json([], "4002", "记录已存在");
        }
        $userId = date('YmdHis').Text::random(null, 4);
    ...
    public function action_MoxieZhixing()
    {
        ...
        if (!isset($this->step[Model_CreditInfo_Step::ZHIXING])){
            $this->model['step']->create(App::$_token['user_id'],Model_CreditInfo_Step::ZHIXING);
        }

        if (isset($this->step[Model_CreditInfo_Step::ZHIXING]) && (int)$this->step[Model_CreditInfo_Step::ZHIXING] == (int)Model_CreditInfo_Step::COMPLETE) {
            $this->response_json([], "4002", "记录已存在");
        }
        $userId = date('YmdHis').Text::random(null, 4);
    ...
    public function action_MoxieEmail()
    {
        ...
        if (!isset($this->step[Model_CreditInfo_Step::EMAIL])){
            $this->model['step']->create(App::$_token['user_id'],Model_CreditInfo_Step::EMAIL);
        }

        if (isset($this->step[Model_CreditInfo_Step::EMAIL]) && (int)$this->step[Model_CreditInfo_Step::EMAIL] == (int)Model_CreditInfo_Step::COMPLETE) {
            $this->response_json([], "4002", "记录已存在");
        }
        $userId = date('YmdHis').Text::random(null, 4);
        ...
        $id = $this->model['moxie_data']->add_data([
            'req_data' => json_encode(array('userId' => $userId)), 'provider' => 'Moxie', 'type' => 'html',
            'state' => 1, 'userid' => $userId, 'tc_no' => '', 'action' => Model_CreditInfo_Step::EMAIL,
        ]);
        $this->model['moxie_data_item']->add_data_item([
            'extends' => App::$_token['user_id'], 'phone' => '', 'check_no' => '', 'tc_no' => '', 'userid' => $userId,
            'state' => 1,
     ...
     * 魔蝎-运营商 （跳出式）
     */
    public function action_MoxieMno()
    {

        $this->step = $this->model['user']->get_ci_step(App::$_token['user_id']);

        if (!isset($this->step[Model_CreditInfo_Step::MNO])){
            $this->model['step']->create(App::$_token['user_id'],Model_CreditInfo_Step::MNO);
        }
        ...
        $id = $this->model['moxie_data']->add_data([
            'req_data' => json_encode(array('userId' => $userId)), 'provider' => 'Moxie', 'type' => 'html',
            'state' => 1, 'userid' => $userId, 'tc_no' => '', 'action' => Model_CreditInfo_Step::MNO,
        ]);
        $this->model['moxie_data_item']->add_data_item([
            'extends' => App::$_token['user_id'], 'phone' => '', 'check_no' => '', 'tc_no' => '', 'userid' => $userId,
            'state' => 1,
        ...
        $rs = $this->model['Risk_QueryRecord']->get_by_array([
            'user_id' => App::$_token['user_id'], 'provider' => 'MoxieMno',
        ], 11);
        if (count($rs) >= 10) {
            $this->response_json([], "4006", "重试次数已超上限");
        }

        $res = Lib::factory('Risk_API')->credit()->html('Mno', [
            'extends' => App::$_token['user_id'],
            'userId' => $userId,
        ])->execute()->body();

        $this->model['Risk_QueryRecord']->create([
            'user_id' => App::$_token['user_id'], 'provider' => 'MoxieMno', 'status' => 1,
        ]);
