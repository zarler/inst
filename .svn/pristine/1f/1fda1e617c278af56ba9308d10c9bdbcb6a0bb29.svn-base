...
class Controller_Ver1_AppHome extends AppCore
{
    ...
    protected function common()
    {
        ...
        if (isset(App::$_token['user_id']) && App::$_token['user_id'] > 0) {//已登录
            ...
            // 授信完成后50天未下单，重新授信（重新认证项不包括应用授权、联系人信息、银行卡认证）
            if ((int)App::$_user['credit_auth'] == Model_User::CREDIT_AUTH_BASE_VERIFIED) {
                ...
                if ($order_num == 0) {
                    if ($pass_time > 0 && $pass_time < (time() - Model_User_Audit::PASS_VALIDITY_TIME)) {
                        $this->expireCreditAuth(50);
                    }
                }else{
                    //首笔订单还款完成后6个月进行重新授信（不包括应用授权、银行卡认证），重新授权并进行风控规则判断，重新出具额度
                    $first_order_repaid_time = $this->model['order']->getFirstRepayTimeByArray(App::$_token['user_id']);
                    $repay_six_month_later = $first_order_repaid_time + Model_User_Audit::ORDER_VALIDITY_TIME;//首次还款后6个月
                    if($first_order_repaid_time > 0 && time() > $repay_six_month_later && $pass_time < $repay_six_month_later){
                        $this->expireCreditAuth(180);
                        Model::factory("CreditInfo_Contact")->cancel(App::$token['user_id']);//联系人信息
            ...
            if ($overdue_num > 0) {
                ...
                $info['button_url'] = $this->_env['url']['app'] . '/v1/v/Loan/Detail?orderId=' . $overdue_order_id;
                ...
                $popup = [
                    ...
                    'popup_left' => '取消',
                    'popup_right' => '去还款',
                    'url' => $this->_env['url']['app'] . '/v1/v/Loan/Detail?orderId=' . $overdue_order_id,
                ];
    ...
    //更改授信状态，并让资料过期
    public function expireCreditAuth($day){
        $this->model['user']->update(App::$_token['user_id'], [
            'credit_auth' => Model_User::CREDIT_AUTH_BASE_READY,
        ]);//授信状态
        Model::factory("CreditInfo_Home")->cancel(App::$token['user_id']);//家庭信息
        Model::factory("CreditInfo_Work")->cancel(App::$token['user_id']);//工作信息
        Model::factory("User_FaceIDAuth")->update(App::$token['user_id'], ['status' => Model_User_FaceIDAuth::STATUS_EXPIRE]);//人脸
        Model::factory('CreditInfo_Step')->overdueModel(App::$_token['user_id'], $day);//其他
