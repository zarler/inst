 ...
 * Time: 下午3:09
 */
class Model_Queue_Payment_UNSPay extends Model_Database
{
...
//    }
    ...
    public function getTotal($array = null) {
        ...
        $query = DB::select(array(DB::expr('COUNT(*)'), 'total'))->from('finance_payment_unspay_queue');
        //处理中
    ...
    public function getList($array = array(), $perpage = 20, $page = 1) {
        $query = DB::select();
        $query->from('finance_payment_unspay_queue');
        //处理中
        if(isset($array['Process']) && $array['Process']){
            $query->where('query_status', 'not in', array(self::QUERY_SUCCESS,self::QUERY_FAILED,self::QUERY_CLOSED,self::QUERY_SUSPENDED));
        }
    ...
    private function queryBuilder($query, $array = array()) {
        ...
        //统一链表
        if (isset($array['join'])) {
            $query->join('order')->on('order.id','=','finance_payment_unspay_queue.order_id');
        }
