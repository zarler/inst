...
class Lib_Risk_API extends Lib_Common
{
    ...
    private function MoxieHtml()
    {
        ...
        Model::factory('Risk_QueryRecord')->create([
            'user_id' => App::$_user['id'], 'provider' => 'Moxie', 'action' => $this->action,
        ]);
...
//        var_dump($result['seq_id']);die;
        Model::factory('AntiFraud_TongDun')->add_tongdun_data([
            'user_id' => $this->param['user_id'], 'module' => 'RiskService', 'request_data' => json_encode($this->param),
     ...
     * 同盾获取详细
     */
    public function TongDunRules()
    {

        $td = new Lib_TONGDUN2_HitRuleDetail($this->param);
        $result = $td->curlGetOpen();
    ...
    */
    ...
    public function ZhiCheng()
    {
        ...
        if (!isset($result['params'])) {
            $this->response(false, [], Lib_Common::LIB_COMMON_API_MISS_PARAM, '缺少参数 params');
        } else {
