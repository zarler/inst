 ...
 * Time: 上午1:09
 *
 * 快金 TOKEN SESSION CACHE v1
 * redis key命名规则 token_session@md5() 取md5后的前两位字符作为key的一部分
 *
 *
 * $array = Lib::factory('TCCache_TokenSession')->token_id(App::$_token['id'])->get();
 * Lib::factory('TCCache_Token')->token_id(App::$_token['id'])->remove();
 *
 *
 *
 *
 */
class Lib_TCCache_TokenSession {

    const PREFIX ='token_session@';   //用户为单位 key: user@123
    ...
    public function token_id($key){
        if(!empty($key)){
            $this->key = (string)self::PREFIX . abs($key%100);//切割100份
            $this->field = $key;
        }
        return $this;
    }
