...
class Controller_Ver1_Inst_Loan extends AppCore
{
    ...
    public function action_CheckStatus()
    {
        ...
        if ($this->CheckStatus()) {
            ...
            $response_array['go_url'] = $this->_env['url']['app'] . '/v1/v/Loan/Check'; //借款
    ...
    public function action_Apply()
    {
        ...
        $today_total = $this->model['Inst']->total_by_date(date('Y-m-d'));
        if(in_array(date('Y-m-d'),['2018-02-12','2018-02-15','2018-02-16','2018-02-17','2018-02-18'])) {
            $this->response_json([], '5122', '抱歉，今日分期申请已抢完,请明日再来');
        }
        if(in_array(date('Y-m-d'),['2018-02-13','2018-02-19','2018-02-20','2018-02-21'])) {
            if($today_total >= 20) {
                $this->response_json([], '5122', '抱歉，今日分期申请已抢完,请明日再来');
            }
        }
        ...
        try {
            ...
            $BillArchive = Model::factory('Bill_Archive');
