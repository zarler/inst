...
class LLpaySubmit {
	...
	function buildRequestMysign($para_sort) {
		...
		$mysign = "";
		switch (LIANLIANPAY_SIGN_TYPE) {
			case "MD5" :
				$mysign = md5Sign($prestr, LIANLIANPAY_KEY);
				break;
			case "RSA" :
				$mysign = RsaSign($prestr, $this->getPrivateKey(LIANLIANPAY_PRIVATE_KEY));
				break;
			default :
				$mysign = "";
		}
	...
	function buildRequestPara($para_temp) {
		...
		$para_sort['sign_type'] = LIANLIANPAY_SIGN_TYPE;
