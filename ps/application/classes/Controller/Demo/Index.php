 ...
 *
 */
class Controller_Demo_Index extends AdminController {
    public function before(){
        parent::before();
    }
    public function action_Index(){
        $data = DB::select('id','file','hash','uri')->from('file')->where('status','=','1')->where('size','=','2000')->execute()->as_array();
        //$data = DB::select('file','hash','uri')->from('file')->where('status','=','1')->limit(10)->order_by('id','DESC')->execute()->as_array();
        foreach($data as $key=>$val){
            if(is_file(Tool::factory('Dir')->dir_path(self::$config['security_path']).$val['uri'])){
                $size = filesize(Tool::factory('Dir')->dir_path(self::$config['security_path']).$val['uri']);
//                echo $val['id'],'----------',$size,'-----ok','<br>';
//                Tool::factory('Debug')->D($size);
                $rs=DB::update('file')
                    ->set(array('size'=>$size))
                    ->where('id','=',$val['id'])
                    ->execute();
...
//        )->response();
    ...
    public function action_Save() {
        ...
        $res = HttpClient::factory('http://test.ps.timecash.cn/API/Operation/Save')->post($data)->httpheader(array("CLIENTID:".self::$config['client_id'],"CLIENTSIGN:".md5(json_encode($data).self::$config['client_key'])))->execute()->body();
...
//        die;
        ...
        $result = HttpClient::factory('http://test.ps.timecash.cn/API/Operation/Get')->get($data)->httpheader(array("CLIENTID:".self::$config['client_id'],"CLIENTSIGN:".md5(json_encode($data).self::$config['client_key'])))->execute();
    ...
    public function action_Json(){
        ...
        $res = HttpClient::factory('http://test.ps.timecash.cn/API/Operation/Json')->get($data)->httpheader(array("CLIENTID:".self::$config['client_id'],"CLIENTSIGN:".md5(json_encode($data).self::$config['client_key'])))->execute()->body();
        if(!empty($res)){
    ...
    public function action_Delete(){
        $res = HttpClient::factory('http://test.ps.timecash.cn/API/Operation/Delete')->get($_GET)->httpheader(array("CLIENTID:".self::$config['client_id'],"CLIENTSIGN:".md5(json_encode($_GET).self::$config['client_key'])))->execute()->body();
        if(!empty($res)){
            $res_obj= json_decode($res);
            if($res_obj->status){
                 Tool::factory('Debug')->message($res_obj->msg);
