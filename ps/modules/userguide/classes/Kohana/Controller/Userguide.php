...
abstract class Kohana_Controller_Userguide extends Controller_Template {
	...
	public function index()
	{
		...
		$this->template->breadcrumb = array('Permission Guide');
	...
	public function error($message)
	{
		...
		if ($module = $this->request->param('module') AND $menu = $this->file($module.'/menu') AND Kohana::$config->load('userguide.modules.'.$module.'.enabled'))
		{
			...
			$this->template->breadcrumb = array(
				$this->guide->uri() => 'Permission Guide',
		...
		elseif (Route::name($this->request->route()) == 'docs/api')
		{
			...
			// Bind the breadcrumb
			$this->template->breadcrumb = array(
				$this->guide->uri(array('page' => NULL)) => 'Permission Guide',
		...
		{
			$this->template->menu = View::factory('userguide/menu',array('modules' => $this->_modules()));
			$this->template->breadcrumb = array($this->request->route()->uri() => 'Permission Guide','Error');
		}
	}
	...
	public function action_docs()
	{
		...
		$breadcrumb = array();
		$breadcrumb[$this->guide->uri()] = 'Permission Guide';
	...
	public function action_api()
	{
		...
		// Add the breadcrumb
		$breadcrumb = array();
		$breadcrumb[$this->guide->uri(array('page' => NULL))] = 'Permission Guide';
