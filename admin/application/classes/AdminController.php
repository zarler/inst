...
class AdminController extends Controller
{
    ...
    function before()
    {
        ...
        $this->user = $this->admin->getUserSessArray();
        $this->group_id = $this->groupId();
        $this->_url   = Request::current()->url();
        ...
        //初始化菜单数据
        $this->_common['menu']['data'] = Model::factory('Admin_Menu')->getAll();
        $this->_common['menu']['tree'] = Model::Factory('Admin_Menu')->getTree($this->_common['menu']['data']);

        $this->self_menu  =  Model::Factory('Admin_Menu')->getMenuByController(array('controller'=>$this->controller,'action'=>$this->action));
        if(!$this->self_menu){
            $this->self_menu  =  Model::Factory('Admin_Menu')->getMenuByController(array('controller'=>$this->controller,'action'=>'List'));
        }

        $this->parent_menu = Model::Factory('Admin_Menu')->getParentMenuByController($this->self_menu['id']);


        Template::bindGlobal('_common',$this->_common);
        Template::bindGlobal('_user',$this->user);
        Template::bindGlobal('_group_id',$this->group_id);
        Template::bindGlobal('_controller',$this->controller);
        Template::bindGlobal('_action',$this->action);
        Template::bindGlobal('parent_menu',$this->parent_menu);
    ...
    {
        return Model::factory('Admin_Group')->getGroupIdByUserId($this->userId());
    }
    ...
    {
        if (isset($this->user['id'])) {
            $main_permission_array = Model::factory('Admin')->getPermissionArray($this->userId());//var_dump($main_permission_array);
    ...
    //
    public function backurlClear($local=FALSE) {
        if($local===TRUE){
