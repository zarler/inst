 ...
 * Time: 下午3:09
 */
class Model_Queue_Deduct extends Model_Database
{
...
//    }
    ...
    public function getTotal($array = null) {
        ...
        $query = DB::select(array(DB::expr('COUNT(*)'), 'total'))->from('finance_deduct_queue') ;
        //处理中
        if(isset($array['Process']) && $array['Process']){
            $query->where('query_status', 'not in', array(self::QUERY_SUCCESS,self::QUERY_FAILED,self::QUERY_CLOSED));
        }
        //扣款成功
        ...
        if(isset($array['Fail']) && $array['Fail']){
            $query->where('query_status', 'in', array(self::QUERY_FAILED,self::QUERY_CLOSED));
        }
        if(count($array)>0) {
    ...
    public function getList($array = array(), $perpage = 20, $page = 1) {
        $query = DB::select('*','finance_deduct_queue.id ids');
        $query->from('finance_deduct_queue');
        //处理中
        if(isset($array['Process']) && $array['Process']){
            $query->where('query_status', 'not in', array(self::QUERY_SUCCESS,self::QUERY_FAILED,self::QUERY_CLOSED));
        }
    ...
    private function queryBuilder($query, $array = array()) {
        ...
        //统一链表
        if (isset($array['join'])) {
            $query->join('order')->on('order.id','=','finance_deduct_queue.order_id');
        }
