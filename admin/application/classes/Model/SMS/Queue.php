...
class Model_SMS_Queue extends Model_Database
{
    ...
    public function create($array = array())
    {
        if (isset($array['user_id']) && isset($array['service_name']) && isset($array['mobile'])) {
            ...
            list($insert_id, $affected_rows) = DB::insert("sms_queue",
                array('user_id', 'service_name', 'mobile', 'status', 'request_data', 'create_time', 'times'))
                ->values($create_array)
                ->execute();
            return $insert_id;
        }
    ...
    public function queueStartHandle($handle = null, $size = 30, $status = Model_SMS_Queue::READY)
    {
        $array = array('status' => Model_SMS_Queue::RUNNING);
        if ($handle !== null) {
            $array['handle'] = $handle;
        }
        return null !== DB::update('sms_queue')->set($array)->where('status', '=', $status)->order_by('id',
                'ASC')->limit(intval($size))->execute();
    }

    //队列 完成启动后,获取占位的处理记录
    public function queueGetRunning($handle = null, $status = Model_SMS_Queue::RUNNING)
    {
    ...
    {
        $affected_rows = DB::update('sms_queue')->set(array(
            'status' => $new_status,
            'response_data' => $response_data,
            'times' => DB::expr('times + 1')
        ))->where('id', '=', intval($id))->execute();
        return $affected_rows !== null;
    }
    ...
    public function getList($array = array(), $perpage = 20, $page = 1)
    {
        ...
        $rs = $query->order_by('sms_queue.id',
