...
class Model_User_FaceIDAuth extends Model_Database {
	...
	public function getList($array=array(),$perpage=20,$page=1) {
		...
		$query = DB::select('user_faceidauth.*',array('user.mobile','mobile'),array('user.id','user_id'),array('user.name','name'),array('user.identity_code','identity_code'),array('user.validated_identity','validated'))->from('user_faceidauth')->join('user','LEFT')->on('user_faceidauth.user_id','=','user.id');
	...
	public function getTotal($array=array()) {
		...
		$query = DB::select(array(DB::expr('COUNT(*)'), 'total'))->from('user_faceidauth')->join('user','LEFT')->on('user_faceidauth.user_id','=','user.id');
