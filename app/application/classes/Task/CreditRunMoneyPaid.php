 ...
 * Time: 上午11:02
 *
 * 由于一些模块已经付钱，所以状态是4的接着跑
 *
 */
class Task_CreditRunMoneyPaid extends Minion_Task
{
    ...
    public function _execute(array $params)
    {
        ...
        $credit_info = $this->model['credit_info']->getByArray([
                                                                   'module' => Model_CreditInfo::MODULE_MONEY_PAID,
                                                                   'result' => Model_CreditInfo::RESULT_UNDO_REJECT,
                                                               ]);
        ...
        foreach ($credit_info as $key => $info) {
            ...
            if (empty($info['module'])) {
                ...
                try {
                    ...
                    //其他规则
                    if ($info['retry'] < Model_CreditInfo::RUN_PAID_MAX_TIMES) {
    ...
    protected function pass()
    {
        ...
        if ($this->credit_model == Model_CreditInfo::ADMISSION || (isset($this->result['api_result']['review']) && $this->result['api_result']['review'] == Model_AntiFraud::PASS)) {
            ...
            $data = [
                'step' => DB::expr('step + 1'),
                'result' => Model_CreditInfo::RESULT_UNDO_REJECT_PASS,
            ];
        } else {
            $data = [
                'step' => DB::expr('step + 1'),
        ...
        $where = ['user_id' => $this->user_id, 'result' => Model_CreditInfo::RESULT_UNDO_REJECT, 'module' => $this->credit_model];
        if (!$this->model['credit_info']->update($where, $data)
        ) {
            throw new Exception('pass > 6失败 ');
    ...
    protected function reject()
    {
        ...
        if (!$this->model['credit_info']->update(['user_id' => $this->user_id, 'result' => 0, 'module' => $this->credit_model], [
            'result' => Model_CreditInfo::RESULT_UNDO_REJECT_REJECT,
        ])
        ) {
            throw new Exception('reject credit_info:result->7失败 ');
        }
    ...
    {
        if (!$this->model['credit_info']->update(['user_id' => $this->user_id, 'result' => 0, 'module' => $this->credit_model], [
            'result' => Model_CreditInfo::RESULT_UNDO_REJECT_HUNG,
        ])
        ) {
            throw new Exception('reject credit_info result => 8失败 ');
        }
