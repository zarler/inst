...
//        $user_id = (int)$user_id;
//        $first_name = isset($array['first_name']) ? mb_substr( trim($ts->filter_utf8($array['first_name'])) , 0, 50, 'utf8') : '';
//        $middle_name = isset($array['middle_name']) ? mb_substr( trim($ts->filter_utf8($array['middle_name'])), 0, 50, 'utf8') : '';
//        $last_name = isset($array['last_name']) ? mb_substr( trim($ts->filter_utf8($array['last_name'])), 0, 50, 'utf8') : '';
//        $address = isset($array['address']) ? mb_substr( trim($ts->filter_utf8($array['address'])), 0, 50, 'utf8') : '';
//        $nick_name = isset($array['nick_name']) ? mb_substr( trim($ts->filter_utf8($array['nick_name'])), 0, 50, 'utf8') : '';
//        $company_name = isset($array['company_name']) ? mb_substr(trim($ts->filter_utf8($array['company_name'])) , 0, 50, 'utf8') : '';
//        $job_title = isset($array['job_title']) ? mb_substr( trim($ts->filter_utf8($array['job_title'])), 0, 50, 'utf8') : '';
//        $birthday = isset($array['birthday']) ? mb_substr( $array['birthday'], 0, 50, 'utf8')  : '';
//        $remark = isset($array['remark']) ? mb_substr( trim($ts->filter_utf8($array['remark'])), 0, 50, 'utf8') : '';
//        $name = isset($array['name']) ? mb_substr( trim($ts->filter_utf8($array['name'])), 0, 60, 'utf8') : '';
//
//        if(!$name){
...
//    }
    ...
    public function create($user_id,$json)
    {
        ...
        list($insert_id,$affected_rows)=DB::insert("app_phonebook_data",array('user_id','phonebook','create_time'))->values(array($user_id,$json,time()))->execute();
        return $insert_id;
    ...
    public function has($user_id){
        return DB::select()->from('app_phonebook_data')->where('user_id','=',(int)$user_id)->limit(1)->execute()->current();
    }
    ...
    public function get_by_user_id($user_id){
        return DB::select()->from('app_phonebook_data')->where('user_id','=',(int)$user_id)->order_by('id','DESC')->execute()->current();
    }

    public function get_by_user_id_name($user_id)
    {
        $data = DB::select('app_phonebook_item.mobile as tel','app_phonebook.name')->from('app_phonebook_item')->join('app_phonebook')->on('app_phonebook.id', '=', 'app_phonebook_item.pbid')
            ->where('app_phonebook_item.user_id','=',(int)$user_id)
            ->where('app_phonebook_item.mobile','is',DB::expr('not null'))
            ->execute()
            ->as_array();
        return $data;
//        return DB::select()->from('app_phonebook_item')->join('')->where('user_id','=',(int)$user_id)->and_where('mobile','is',DB::expr('not null'))->execute()->as_array();
    }
