...
class Lib_Array
{
    ...
    /* 用法介绍
     * 过滤数组留下需要的字段
     * $columns = Lib::factory('Array')->key_filter(array('user_id'=>1,'mobile'=>'13333331233','email'=>'aaa@aaa.cn'),array('user_id','mobile','identity_code'));
     * 只留需要的字段,如果不存在给空字符值.
     * $columns = Lib::factory('Array')->key_filter($array,array('user_id','mobile'),'');
     * 过虑数组,给不同的键配置值独立的默认值
     * $columns = Lib::factory('Array')->key_filter($array,array('user_id','mobile'),array('user_id'=>0,'mobile'=>'00000000000'));
     */
    //过滤数组字段,只取允许的键,最后一个参数不为NULL时将给给不存在的字段设置默认值.
    public static function keyFilter($search_array, $key, $def=NULL) {
    ...
    //过滤数组字段,只取允许的并且有值的键
    public static function keyFilterHasValue($search_array, $key)
    {
        if(!is_array($search_array)){
            return FALSE;
        }
    ...
    //数组值转字符串
    public function valueString($array = NULL)
    {
        if(is_array($array)){
    ...
    public function useSortarrTosortArray($sortarr,$arr)
    {
        $_arr=array();
     ...
     */
    public function arrayValue2key($arr,$field="id")
    {
        if(!empty($arr)){
            foreach($arr as $k => $v){
                $res[$v[$field]] = $v;
