...
function getHttpResponseJSON($url, $para) {
    ...
    //记录请求
    $log->write(array(array('body'=>$url,'time'=>time())),"time [{$id}] - IP:{$ip} --- LIANLIANPAY REQUEST: \r\nbody \r\n");
    $responseText = curl_exec($curl);
    //记录返回信息
    $log->write(array(array('body'=>$responseText,'time'=>time())),"time [{$id}] --- LIANLIANPAY RESPONSE: \r\nbody\r\n");
    //file_put_contents("log.txt","返回值:".$responseText."\n", FILE_APPEND);
    curl_close($curl);
    return $responseText;
}
