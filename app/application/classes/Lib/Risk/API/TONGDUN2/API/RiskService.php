 ...
 * Time: 15:48
 */
class Lib_Risk_API_TONGDUN2_API_RiskService {
    //通用数据
    private $event_id;
...
//        var_dump($this->defaulData);die;
    ...
    function curlOpen($api_url, array $params) {
        ...
        //记录发送数据
        $log->write(array(array('body' => json_encode($params), 'time' => time())),
            "time [{$id}] - IP:{$ip} ---  REQUEST: \r\nbody \r\n");
        ...
        $ip = Request::$client_ip;
        //记录发送数据
        $log->write(array(array('body' => json_encode($response), 'time' => time())),
            "time [{$id}] - IP:{$ip} ---  REQUEST: \r\nbody \r\n");
